...
class JoiJsonSchemaParser {
  ...
  _removeStrippedFields(joiSchema) {
    if (joiSchema?.flags?.result === 'strip') {
      return undefined
    }
    switch (joiSchema.type) {
      ...
      case 'array':
        return {
          ...joiSchema,
          items: joiSchema.items.map(item => this._removeStrippedFields(item)).filter(x => x)
        }
      ...
      case 'object':
        const strippedKeys = {}
        if (joiSchema.keys) {
          for (const key of Object.keys(joiSchema.keys)) {
            const stripped = this._removeStrippedFields(joiSchema.keys[key])
            if (stripped) {
              strippedKeys[key] = stripped
            }
          }
        }
  ...
  parse(joiSpec, definitions = {}, level = 0) {
    ...
    joiSpec = this._removeStrippedFields(joiSpec)
    if (!joiSpec) {
      return schema
    }
